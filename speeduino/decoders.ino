...
/* -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Name: EDIS4
todo: a description here
*/
void triggerSetup_EDIS4()
{
  triggerActualTeeth = configPage2.nCylinders;
  if(triggerActualTeeth == 0) { triggerActualTeeth = 1; }
  triggerToothAngle = 720 / triggerActualTeeth; //The number of degrees that passes from tooth to tooth
...
void triggerPri_EDIS4()
{
  ...
  if ( (curGap >= triggerFilterTime) )
  {
    ...
    EDISIgnition = currentStatus.advance;
    if (EDISIgnition < 0)  {EDISIgnition = 0;}   //limitations due to EDIS hardware...there is a possibility to set this to -10...to....59. Imnon sure.
    if (EDISIgnition > 59)  {EDISIgnition = 59;}   
    EDISDuration = 1536 - (25.6 * EDISIgnition);

    setIgnitionSchedule1(ign1StartFunction, 10, EDISDuration, ign1EndFunction);   //set an output schedule to do an output routine to IGN1 pin , 10us far from here and duration equal to adequate ign timing.
...
void triggerSec_EDIS4() { return; } //Not required
uint16_t getRPM_EDIS4()
{
  uint16_t tempRPM;
  if( currentStatus.RPM < currentStatus.crankRPM)
  { 
